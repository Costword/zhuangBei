...
static CGFloat const PPStickerTextViewHeight = 54.0;
...
//    self.sendBtn.frame = CGRectMake(self.bounds.size.width - 70, (self.textView.bounds.size.height - 35)/2+10, 60, 35);
    self.emojiToggleButton.frame = CGRectMake(self.bounds.size.width - 70, (self.textView.bounds.size.height - 35)/2+10, 35, 35);
    [self refreshTextUI];
}
...
- (CGFloat)heightThatFits
{
    if (self.keepsPreModeTextViewWillEdited) {
        ...
        CGFloat height = PPStickerTextViewTextViewTopMargin + calculateHeight + PPStickerTextViewTextViewBottomMargin ;
        return height;
    }
}
...
- (void)setCustomPlainText:(NSString *)msg
{
    self.textView.placeholder = msg;
}
...
{
    if (!_textView) {
        _textView = [[PPStickerTextView alloc] initWithFrame:CGRectMake(15, 0, self.bounds.size.width, 70)];
...
//        _textView.placeholder = @"发送消息";
        ...
        [_textView setBoundWidth:1 cornerRadius:4 boardColor:BASECOLOR_BOARD];
...
- (CGRect)frameTextView
{
    ...
    CGFloat width = self.bounds.size.width - (2 * minX)-80;

    CGFloat height = 0;
    if (self.keepsPreModeTextViewWillEdited) {
        height = CGRectGetHeight(self.bounds) - 2 * PPStickerTextViewTextViewTopMargin;
    } else {
        height = CGRectGetHeight(self.bounds) - PPStickerTextViewTextViewTopMargin - PPStickerTextViewTextViewBottomMargin ;
    }
    if (height < 0) {
...
- (void)stickerKeyboard:(PPStickerKeyboard *)stickerKeyboard didClickEmoji:(PPEmoji *)emoji
{
    ...
    NSRange selectedRange = self.textView.selectedRange;
    NSString *emojiString = [NSString stringWithFormat:@"face[%@]", emoji.emojiDescription];
